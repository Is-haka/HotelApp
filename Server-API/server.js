...
server.post("/booking/create", (req, res) => {
  ...
  server.post("/booking/create", (req, res) => {
    ...
    const recentBookings = req.session.bookings.filter(
      (booking) => now - new Date(booking.timestamp) < bookingInterval
    );
    if (recentBookings.length >= bookingLimit) {
      return res.send({
        status: false,
        message: "Booking limit reached. Try again later.",
      });
    }
...
server.get("/hotel", (req, res) => {
  ...
  connection.query(qry, [regionId], (error, result) => {
    if (error) {
      ...
      res
        .status(500)
        .send({ status: false, message: "Unable to fetch hotels." });
    } else {
...
server.get("/region", (req, res) => {
  ...
  if (!regionName) {
    return res
      .status(400)
      .send({ status: false, message: "Region name is required" });
  }
  ...
  connection.query(query, [`%${regionName}%`], (error, results) => {
    if (error) {
      ...
      return res
        ...
        .send({
          status: false,
          message: "An error occurred while searching for regions",
        });
    }
    ...
    if (results.length > 0) {
      ...
      res.send({
        status: false,
        message: "No regions found for the specified name",
      });
    }
  });
});
  ...
  connection.query(qry, [`%${bookingNo}%`], (error, results) => {
    if (error) {
      ...
      return res
        ...
        .send({
          status: false,
          message: "An error occurred while searching for booking number",
        });
    }
    ...
    if (error) {
      ...
      return res
        ...
        .send({
          status: false,
          message:
            "An error occurred while fetching data of the particular book number",
        });
    }
...
server.get("/relatedType", (req, res) => {
  ...
  connection.query(qry, [`${compId}`], (error, results) => {
    if (error) {
      ...
      return res
        ...
        .send({
          status: false,
          message:
            "Error occurred while fetching the related complain using initial complain",
        });
    }
    ...
    if (results.length > 0) {
      ...
      res.send({
        status: false,
        message: "No more complain for that question",
      });
    }
  });
});
...
server.get("/relatedAnswer", (req, res) => {
  ...
  connection.query(qry, [`${relatedCompId}`], (error, results) => {
    if (error) {
      ...
      return res
        ...
        .send({
          status: false,
          message:
            "Error occurred while fetching the related complain answer using initial complain",
        });
    }
    ...
    if (results.length > 0) {
      ...
      res.send({
        status: false,
        message: "No more complain answer for that question",
      });
    }
  });
});
  ...
  connection.query(query, [wildcardRegionName], (error, results) => {
    if (error) {
      ...
      return res
        ...
        .send({
          status: false,
          message: "An error occurred while fetching region ID",
        });
    }
...
server.get("/available", (req, res) => {
  ...
  connection.query(qry, [region_id], (error, result) => {
    if (error) {
      ...
      return res
        .status(500)
        .send({ status: false, message: "Unable to fetch room availability." });
    }
...
server.get("/inquiryAnswer", (req, res) => {
  ...
  if (!answer) {
    return res
      .status(400)
      .send({ status: false, message: "inquiry_type ID is required." });
  }
  ...
  connection.query(qry, [answer], (error, result) => {
    if (error) {
      ...
      return res
        .status(500)
        .send({ status: false, message: "Unable to fetch answer" });
    }
    console.log(result);
    return res
      .status(201)
...
server.get("/answer", (req, res) => {
  ...
  if (!answer) {
    return res
      .status(400)
      .send({ status: false, message: "complain_type ID is required." });
  }
...
server.post("/inquiry", (req, res) => {
  ...
  if (customDesc && fullname && phone) {
    const qry =
      "INSERT INTO complain(fullname, phone, typeOfInquiry, customDesc) VALUES(?, ?, ?, ?)";
    ...
    return res
      .status(400)
      .send({ status: false, message: "Please enter your inquiry" });
  }
});
...
server.get("/rooms", (req, res) => {
  ...
  if (!hotelId) {
    return res
      .status(400)
      .send({ status: false, message: "Hotel ID is required." });
  }
  ...
  connection.query(qry, [hotelId], (error, result) => {
    if (error) {
      ...
      return res
        .status(500)
        .send({ status: false, message: "Unable to fetch rooms." });
    }
