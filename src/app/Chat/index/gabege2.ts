...
userForm!: FormGroup;
  ...
  */
  ...
  //method to check what start date is being selected before freezing itbooking in the end date form fields
...
chooseOption(option: string) {
                ...
                .subscribe(
                  (response: any) => {
                    if (response.status) {
                      ...
                      this.cookieService.delete('bookingTokens'); // Clear cookie after successful booking
                      this.cookieService.delete('lastBookingTime'); // Clear cookie after successful booking
  ...
  canBook(now: Date): boolean {
    const cookieName = 'bookingTokens';
    const cookieValue = this.cookieService.get(cookieName);
    const lastBookingTimeCookie = this.cookieService.get('lastBookingTime');

    this.bookingTokens = cookieValue ? parseInt(cookieValue, 10) : 0;
    this.lastBookingTime = lastBookingTimeCookie ? new Date(lastBookingTimeCookie) : null;
  ...
  updateTokenTracking(now: Date): void {
    const cookieName = 'bookingTokens';
    this.bookingTokens = this.bookingTokens + 1;
    this.cookieService.set(cookieName, this.bookingTokens.toString(), 5/60, '/'); // Set cookie for 5 minutes
    this.cookieService.set('lastBookingTime', now.toISOString(), 5/60, '/'); // Set cookie for 5 minutes
  }
